...
\State {\bf Initialization:} Set $B_{0}:=A$, $\gamma=\big\lfloor \dfrac{1}{2}\log_{2} \dfrac{T}{e}\big\rfloor $, $m:=0$, $\epsilon_{0}:=1$ and $N_{0}=\big\lceil \dfrac{\log(\psi T\epsilon_{0}^{2})}{\epsilon_{0}} \big\rceil$.
...
\State $N_{m+1} := N_{m} + \big\lceil \dfrac{\log(\psi T\epsilon_{m}^{2})}{\epsilon_{m}} \big\rceil$
