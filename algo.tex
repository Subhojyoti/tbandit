...
\State {\bf Initialization:} Set $B_{0}:=A$, $M=\big\lfloor \dfrac{1}{2}\log_{2} \dfrac{T}{e}\big\rfloor $, $m:=0$, $\epsilon_{0}:=1$, $n_{0}=\big\lceil \dfrac{2\log(\psi T\epsilon_{0}^{2})}{\epsilon_{0}} \big\rceil$ and $N_{0}=K*n_{0} $.
